...
const Training = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <section className={`relative ${isMobile ? 'h-[60vh]' : 'h-[650px]'} overflow-hidden`}>
        {/* Slides Container */}
        <div className="relative w-full h-full">
          {heroSlides.map((slide, index) => (
            ...
            >
              ...
              {/* Slide Content - BOLD TITLES ONLY HERE */}
              <div className={`relative z-10 h-full flex items-center justify-center ${getTextAlignment(slide.position)}`}>
                <div className="container mx-auto px-4">
                  <div className={`${isMobile ? 'max-w-full' : 'max-w-3xl'} ${slide.position === 'center' ? 'mx-auto' : slide.position === 'right' ? 'ml-auto' : ''}`}>
                    <h1 className={`${isMobile ? 'text-2xl md:text-3xl' : 'text-4xl md:text-5xl'} font-extrabold text-white mb-4 leading-snug`}>
                      {slide.title}
                    </h1>
                    <p className={`${isMobile ? 'text-base md:text-lg' : 'text-xl md:text-2xl'} text-white mb-6 leading-relaxed`}>
                      {slide.description}
                    </p>
                    <div className={slide.position === 'center' ? 'flex justify-start' : 'flex justify-start'}>
                      <Button
                        ...
                        className="bg-[#081748] text-white hover:bg-blue-800 font-semibold px-6 py-3 text-base shadow-lg hover:shadow-xl transition-all"
                      >
      ...
      <section id="formats" className="py-8 md:py-16 bg-blue-100">
        <div className="container mx-auto px-4">
                ...
                {formatCards.map((format) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-base text-[#081748]">{format.title}</h3>
                  ...
                  <div className="bg-white rounded-2xl w-full max-w-md max-h-[80vh] overflow-hidden">
                    ...
                    <div className="p-6 overflow-y-auto">
                      ...
                      <div className="space-y-4 mb-6">
                        <h4 className="font-semibold text-[#081748]">What's Included:</h4>
      ...
      <section id="content" className="py-8 md:py-16 bg-blue-100">
        <div className="container mx-auto px-4">
              ...
              <div key={course.id} className="bg-background rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                <div className={isMobile ? "flex" : ""}>
                  <img 
                    src={course.image} 
                    alt={course.title} 
                    className={`${isMobile ? 'w-32 h-32 object-cover' : 'w-40 h-44 object-cover'}`}
                  />
                  <div className={`${isMobile ? 'flex-1' : ''} p-4 space-y-2`}>
                    <h3 className={`${isMobile ? 'text-2xl' : 'text-xl'} font-bold text-foreground`}>
                      {course.title}
                    </h3>
                    <p className={`${isMobile ? 'text-sm' : 'text-base'} text-muted-foreground line-clamp-3`}>
                      {course.description}
                    </p>
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <div className={`${isMobile ? 'text-lg' : 'text-xl'} font-semibold text-primary`}>
                          {getConvertedPrice(course.price)}
                        </div>
                        {course.groupPrice && (
                          <div className={`${isMobile ? 'text-sm' : 'text-base'} text-green-600`}>
      ...
      {/* Contact Section */}
      <section id="contact" className="py-10 md:py-16 bg-blue-100 text-black">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-left space-y-6">
            <div className={`${isMobile ? 'w-14 h-14' : 'w-16 h-16'} rounded-full bg-blue-100 flex items-center justify-center shadow-lg`}>
              <Award className={`${isMobile ? 'w-7 h-7' : 'w-8 h-8'} text-black`} />
            </div>
            <h2 className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-semibold`}>
              Certificate Inquiry
            </h2>
            <p className={`${isMobile ? 'text-base' : 'text-lg'} text-black`}>
              Consult us for your certified training needs. Get in touch to learn more.
            </p>
            <div className="flex flex-col sm:flex-row gap-3">
              <Button 
                size={isMobile ? "default" : "lg"}
                className="bg-blue-100 text-black hover:bg-blue-900 hover:text-[#081748] px-6 py-3 shadow-lg hover:shadow-xl transition-all duration-300"
